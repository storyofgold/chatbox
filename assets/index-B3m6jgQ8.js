(function(){const i=document.createElement("link").relList;if(i&&i.supports&&i.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))n(e);new MutationObserver(e=>{for(const t of e)if(t.type==="childList")for(const r of t.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function s(e){const t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?t.credentials="include":e.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function n(e){if(e.ep)return;e.ep=!0;const t=s(e);fetch(e.href,t)}})();const I={x:.3,y:4,width:100-.4-5.6,height:86},D=async a=>({inlineData:{data:await new Promise((n,e)=>{const t=new FileReader;t.onloadend=()=>{typeof t.result=="string"?n(t.result.split(",")[1]):e(new Error("Failed to read file as string."))},t.onerror=r=>{e(r)},t.readAsDataURL(a)}),mimeType:a.type}}),F=a=>{if(!a)return[];const i=a.split(`
`).map(n=>n.trim()).filter(Boolean),s=[];for(const n of i)try{if(n.toUpperCase().startsWith("LINE:")){const e=/x1=([\d.]+).*y1=([\d.]+).*x2=([\d.]+).*y2=([\d.]+).*color=([^,]+).*strokeWidth=([\d.]+)/i.exec(n);e&&s.push({type:"line",x1:+e[1],y1:+e[2],x2:+e[3],y2:+e[4],color:e[5].trim(),strokeWidth:+e[6]||.3})}else if(n.toUpperCase().startsWith("RECT:")){const e=/x=([\d.]+).*y=([\d.]+).*width=([\d.]+).*height=([\d.]+).*color=([^,]+).*fillOpacity=([\d.]+)/i.exec(n);e&&s.push({type:"rect",x:+e[1],y:+e[2],width:+e[3],height:+e[4],color:e[5].trim(),fillOpacity:+e[6]||.2})}else if(n.toUpperCase().startsWith("TEXT:")){const e=/x=([\d.]+).*y=([\d.]+).*text=\"([^\"]+)\".*color=([^,]+).*fontSize=([\d.]+)/i.exec(n);e&&s.push({type:"text",x:+e[1],y:+e[2],text:e[3],color:e[4].trim(),fontSize:+e[5]||1.5})}}catch(e){console.error("Failed to parse annotation line:",n,e)}return s},R=(a,i,s)=>{a.innerHTML="";const n=document.createElementNS("http://www.w3.org/2000/svg","g");n.setAttribute("transform",`translate(${s.x} ${s.y}) scale(${s.width/100} ${s.height/100})`),i.forEach(e=>{let t;e.type==="line"?(t=document.createElementNS("http://www.w3.org/2000/svg","line"),t.setAttribute("x1",String(e.x1)),t.setAttribute("y1",String(e.y1)),t.setAttribute("x2",String(e.x2)),t.setAttribute("y2",String(e.y2)),t.setAttribute("stroke",e.color||"#ffffff"),t.setAttribute("stroke-width",String(e.strokeWidth))):e.type==="rect"?(t=document.createElementNS("http://www.w3.org/2000/svg","rect"),t.setAttribute("x",String(e.x)),t.setAttribute("y",String(e.y)),t.setAttribute("width",String(e.width)),t.setAttribute("height",String(e.height)),t.setAttribute("fill",e.color||"#ffffff"),t.setAttribute("fill-opacity",String(e.fillOpacity))):e.type==="text"&&(t=document.createElementNS("http://www.w3.org/2000/svg","text"),t.setAttribute("x",String(e.x)),t.setAttribute("y",String(e.y)),t.setAttribute("fill",e.color||"#ffffff"),t.setAttribute("font-size",String(e.fontSize)),t.textContent=e.text),t&&n.appendChild(t)}),a.appendChild(n)};document.addEventListener("DOMContentLoaded",()=>{const a=document.getElementById("analysis-form"),i=document.getElementById("chart-upload"),s=document.getElementById("upload-button"),n=document.getElementById("file-name-display"),e=document.getElementById("user-query-input"),t=document.getElementById("analyze-button"),r=document.getElementById("analysis-history"),S=document.querySelector(".placeholder");let h=null;if(a&&i&&s&&n&&e&&t&&r){const g=()=>{e!=null&&e.value.trim()?t.disabled=!1:t.disabled=!0},x=o=>{o&&o.type.startsWith("image/")?(h=o,n.textContent=o.name,s.classList.add("file-selected")):(h=null,n.textContent="",s.classList.remove("file-selected"),o&&alert("Please select an image file (e.g., PNG, JPG).")),g()};s.addEventListener("click",()=>i.click()),i.addEventListener("change",()=>{const o=i.files?i.files[0]:null;x(o)}),e.addEventListener("input",()=>{g(),e.style.height="auto",e.style.height=`${e.scrollHeight}px`}),r.addEventListener("dragover",o=>{o.preventDefault(),r.classList.add("drag-over")}),r.addEventListener("dragleave",o=>{o.preventDefault(),r.classList.remove("drag-over")}),r.addEventListener("drop",o=>{var l;o.preventDefault(),r.classList.remove("drag-over");const u=(l=o.dataTransfer)!=null&&l.files?o.dataTransfer.files[0]:null;u&&(i.files=o.dataTransfer.files,x(u))}),a.addEventListener("submit",async o=>{o.preventDefault();const u=e.value.trim();if(!u){alert("Please enter a query.");return}S&&(S.style.display="none");const l=h,p=document.createElement("div");p.className="analysis-entry";const v=document.createElement("p");v.className="user-query-text",v.textContent=l?`Your query for "${l.name}": "${u}"`:`Your question: "${u}"`;const y=document.createElement("div");y.className="loader",p.appendChild(v);let m=null;if(l){const c=document.createElement("div");c.className="image-container";const d=document.createElement("img");d.src=URL.createObjectURL(l),d.alt="Uploaded chart screenshot for analysis",m=document.createElementNS("http://www.w3.org/2000/svg","svg"),m.setAttribute("class","annotation-overlay"),m.setAttribute("viewBox","0 0 100 100"),m.setAttribute("preserveAspectRatio","none"),c.appendChild(d),c.appendChild(m),p.appendChild(c)}p.appendChild(y),r.appendChild(p),r.scrollTop=r.scrollHeight,x(null),i.value="",e.value="",e.style.height="auto",g();try{const E=(await(await fetch("https://vigorously-fun-yak.ngrok-free.app/query",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({})})).json()).reference,k=await(await fetch("https://vigorously-fun-yak.ngrok-free.app/api-query",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:"Apa ibu kota Indonesia?"})})).json();if(console.log(k.text),l){const w=await D(l),A=`You are an expert financial market analyst specializing in Wyckoff methodology.
...
Begin your full analysis now.`,f=(await ai.models.generateContent({model:"gemini-2.5-flash",contents:{parts:[w,{text:A}]}})).text,N="--- ANNOTATIONS ---",T=f.indexOf(N);let O=f,L="";T!==-1?(O=f.substring(0,T).trim(),L=f.substring(T+N.length).trim()):console.warn("Annotation separator not found in the response.");const b=document.createElement("div");if(b.className="analysis-text",b.innerHTML=O.replace(/\n/g,"<br>"),m){const P=F(L);R(m,P,I)}p.replaceChild(b,y)}else{const w=`You are an expert financial market analyst specializing in Wyckoff methodology.
